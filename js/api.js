...
window.onload = async function(){
    const urlParams = new URLSeatchParams(window.location.search).get('board_type')
}
게시글 상세  이동시 
window.location.href = `${BACK_BASE_URL}/board/?board_type=게시판종류 & board_id=게시글id`
window.onload = async function(){
    const urlParams = new URLSeatchParams(window.location.search).get('board_id')
}
 */
export async function injectNavbar(){
    fetch(`${FRONT_BASE_URL}/html/nav_bar.html`).then(response=>{
        return response.text()
    })
    .then(data=>{
        document.querySelector("header").innerHTML = data;
    })
    let navbarHtml = await fetch("./nav_bar.html")
